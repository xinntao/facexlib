...
class FaceRestoreHelper(object):
    ...
    def get_face_landmarks_5(self, only_keep_largest=False, only_center_face=False, pad_blur=False):
        ...
        # pad blurry images
        # Assumption: only one face is detected
        if pad_blur:
            assert len(self.all_landmarks_5) == 1, 'Only support one face now'
            # TODO: support multi faces
            # get landmarks
            eye_left = self.all_landmarks_5[0][0, :]
            eye_right = self.all_landmarks_5[0][1, :]
            eye_avg = (eye_left + eye_right) * 0.5
            mouth_avg = (self.all_landmarks_5[0][3, :] + self.all_landmarks_5[0][4, :]) * 0.5
            ...
            if max(pad) > 1:
                # pad image
                self.input_img = np.pad(self.input_img, ((pad[1], pad[3]), (pad[0], pad[2]), (0, 0)), 'reflect')
                # modify landmark coords
                self.all_landmarks_5[0][:, 0] += pad[0]
                self.all_landmarks_5[0][:, 1] += pad[1]
                # blur pad images
                h, w, _ = self.input_img.shape
                ...
                if blur % 2 == 0:
                    blur += 1
                # use boxFilter (instead of Gaussian filter) to speed up
                blur_img = cv2.boxFilter(self.input_img, 0, ksize=(blur, blur))

                self.input_img = self.input_img.astype('float32')
                self.input_img += (blur_img - self.input_img) * np.clip(mask * 3.0 + 1.0, 0.0, 1.0)
                self.input_img += (np.median(self.input_img, axis=(0, 1)) - self.input_img) * np.clip(mask, 0.0, 1.0)
                self.input_img = np.clip(self.input_img, 0, 255)  # float32, [0, 255]
